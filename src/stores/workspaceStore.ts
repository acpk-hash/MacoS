...
export const useWorkspaceStore = create<WorkspaceStore>((set, get) => ({
  ...
  listDir: async (rel) => {
    ...
    try {
      const entries = await tauriInvoke<WsEntry[]>('ws_list_dir', { relPath: rel })
      set((s) => {
  ...
  openFile: async (rel, name) => {
    ...
    try {
      const res = await tauriInvoke<WsFileContent>('ws_read_file', { relPath: rel })
  ...
  saveTab: async (rel) => {
    ...
    try {
      await tauriInvoke<void>('ws_write_file', { relPath: rel, content: tab.content })
      set((s) => ({
        tabs: s.tabs.map((t) =>
  ...
  createNode: async (parent, name, isDir) => {
    ...
    try {
      await tauriInvoke<void>('ws_create', { relPath: rel, isDir })
  ...
  deleteNode: async (rel, isDir) => {
    if (!isTauri) return
    try {
      await tauriInvoke<void>('ws_delete', { relPath: rel })
  ...
  renameNode: async (rel, nextName) => {
    ...
    if (to === rel) return
    try {
      await tauriInvoke<void>('ws_rename', { from: rel, to })
      await get().listDir(parent)
      set((s) => ({
        tabs: s.tabs.map((t) =>
  ...
  applyAiTouched: async (paths) => {
    const { root, tabs, children, expanded } = get()
