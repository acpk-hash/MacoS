...
// -- Built-in free provider (Agnes via Sub2API panel) --------------------------

/// Well-known id for the built-in Agnes provider (never collides with UUIDs).
const BUILTIN_AGNES_ID: &str = "builtin-agnes";
const BUILTIN_AGNES_LABEL: &str = "Iris Free (Agnes)";
const BUILTIN_AGNES_BASE_URL: &str = "http://107.174.70.15:8080/v1";
...
pub(crate) fn classify_model(id: &str) -> &'static str {
    ...
    if l.contains("sora") || l.contains("veo-") || l == "agnes-video-v2.0" {
        return "video";
    }
    if l.contains("dall-e")
        || l.contains("dalle")
        || l.contains("flux")
        || l.contains("midjourney")
        || l.starts_with("agnes-image-")
    {
        return "image";
    }
    // Generic "image" substring check AFTER agnes-image was already caught,
    // so agnes-2.0-flash (no "image" substring) stays chat.
    if l.contains("image") {
        return "image";
    }
    "chat"
